...
const renderItem = function({ item }, navigation) {
  return (
    <TouchableOpacity
      onPress={ function() {
        navigation.navigate("UpdateDanHapusSupplier", {
    ...
    >
      <View style={{ flexDirection: "row", flexWrap: "wrap"}}>
        <View style={{ marginBottom: 5 }}>
          <Text style={{ fontSize: 16 }}>Nama: {item.nama} | Alamat: {item.alamat} | Email: {item.email} | Nomor Telepon: {item.nomor_telepon}</Text>
        </View>
      </View>
    </TouchableOpacity>
  );
...
const DataSupplierScreen = function({ navigation }) {
  ...
  React.useEffect(function() {
    (async function() {
      ...
      setListSupplier(data.listSupplier);
    })();
  }, [listSupplier]);
...
export default DataSupplierScreen;